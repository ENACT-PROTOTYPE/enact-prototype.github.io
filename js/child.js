...
function(a, b, c) {
    ...
    d.Defaults = { autoplay: !1, autoplayTimeout: 5e3, autoplayHoverPause: !1, autoplaySpeed: !1 }, d.prototype.autoplay = function() { this.core.settings.autoplay && !this.core.state.videoPlay ? (b.clearInterval(this.interval), this.interval = b.setInterval(a.proxy(function() { this.play() }, this), this.core.settings.autoplayTimeout)) : b.clearInterval(this.interval) }, d.prototype.play = function() {
        ...
        for (a in this.handlers) this.core.$element.off(a, this.handlers[a]);
        for (c in Object.getOwnPropertyNames(this)) "function" != typeof this[c] && (this[c] = null) }, a.fn.owlCarousel.Constructor.Plugins.autoplay = d }(window.Zepto || window.jQuery, window, document),
...
jQuery(document).ready(function($) {
    ...
    $('.type-download').matchHeight();

    
    $(".edd_price_options ul li:first-child").hover(
      function () {
        $(this).addClass("single-tooltip");
      },
      function () {
        $(this).removeClass("single-tooltip");
      }
    );
    $(".edd_price_options ul li:nth-child(2)").hover(
      function () {
        $(this).addClass("multiple-tooltip");
      },
      function () {
        $(this).removeClass("multiple-tooltip");
      }
    );
    $(".edd_price_options ul li:nth-child(3)").hover(
      function () {
        $(this).addClass("extended-tooltip");
      },
      function () {
        $(this).removeClass("extended-tooltip");
      }
    );

    $(".tooltip-license").hover(
      function () {
        $('#tooltip-license-popup').show();
      },
      function () {
        $('#tooltip-license-popup').hide();
      }
    );
    
    $('input[name="edd_options[price_id][]"]').change(function() {
        var data = $('input[name="edd_options[price_id][]"]:checked').attr('class');
        var num = $('input[name="edd_options[price_id][]"]:checked').val();
        resetSelectPrice(data);
        $('#'+data+"_"+num).parents('li').addClass("active");
    });

    function resetSelectPrice(id){
        $('.edd_price_options ul li.active').removeClass("active");
    }

    if($('input[name="edd_options[price_id][]"]:checked').length > 0){
        var data = $('input[name="edd_options[price_id][]"]:checked').attr('class');
        var num = $('input[name="edd_options[price_id][]"]:checked').val();        
        $('#'+data+"_"+num).parents('li').addClass("active");
    }

    $(".popup-login a, a.popup-login").on('click',function(event){
        event.preventDefault();
        $('#login-popup').fadeIn();
    });

    /*$(".ticket-login a").on('click',function(event){
        event.preventDefault();
        $('#ticket-login').fadeIn();
    });*/

    $(".login-popup-close").on('click',function(event){
        event.preventDefault();
        $('#login-popup').fadeOut();
    });

    /*$(".ticket-login-popup-close").on('click',function(event){
        event.preventDefault();
        $('#ticket-login').fadeOut();
    });*/

    $(".sharing-popup-close").on('click',function(event){
        event.preventDefault();
        $('#open-sharing').fadeOut();
    });

    $(".download-with-sharing").on('click',function(event){
         event.preventDefault();
        $('#open-sharing').fadeIn();
    });
    ...
    $('#show-register').on('click',function(){
        $('#login-form').css('position','absolute').css('width','100%');;
    ...
    $('#login-show').on('click',function(){
        ...
        $('#register-form').css('position','absolute').css('width','100%');
    });

    $('#ticket-show-register').on('click',function(){
        $('#ticket-form').css('position','absolute').css('width','100%');;
        $('#ticket-register-form').fadeIn(1000);
        $('#ticket-form').fadeOut(2000);
        $('#ticket-register-form').css('position','relative');
    });

    $('#ticket-login-show').on('click',function(){
        $('#ticket-register-form').fadeOut(1000);
        $('#ticket-form').fadeIn(500);
        $('#ticket-form').css('position','relative');
        $('#ticket-register-form').css('position','absolute').css('width','100%');
    });

    $('#edd-reviews-review-form-submit').on('click',function(){
        if($('#edd-reviews-star-rating').val() == ""){
            alert("Rating");
            return false;
        }
    });
    $(".loxlox-price").text(function () {
        return $(this).text().replace("$0", "Free"); 
    });
